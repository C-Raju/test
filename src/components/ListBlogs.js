...
const ListBlogs = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await fetch("https://mern-1xrisztsn-c-raju.vercel.app/blog/all", {
          method: "GET",
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`https://mern-1xrisztsn-c-raju.vercel.app/blog/delete/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          token: localStorage.getItem("token"),
